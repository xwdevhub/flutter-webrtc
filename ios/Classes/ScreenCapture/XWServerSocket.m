...
@interface XWServerSocket()<GCDAsyncSocketDelegate>
...
@implementation XWServerSocket
+ (XWServerSocket *)shared{
    static XWServerSocket *shareInstance = nil;
...
- (void)onRecvData:(NSData *)data
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        XWI420Frame *frame = [XWI420Frame initWithData:data];
        CMSampleBufferRef sampleBuffer = [frame convertToSampleBuffer];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.onBufferReceived) {
